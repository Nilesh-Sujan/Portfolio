...
function() {
    ...
    if (e != this._quick_info_box_id && this.config.show_quick_info) {
        ...
        t && (this._quick_info_box = this._init_quick_info(t),
            this._fill_quick_data(e), this._show_quick_info(t))
    }
    ...
    if (this._quick_info_box_id = 0, t && t.parentNode) {
        var a = t._offsetWidth;
        if (scheduler.config.quick_info_detached) return t.parentNode.removeChild(t);
        "auto" == t.style.right ? t.style.left = -a + "px" : t.style.right = -a + "px", e && t.parentNode.removeChild(t)
    }
...
scheduler._show_quick_info = function(e) {
    ...
    }, 1)), t.className = t.className.replace("dhx_qi_left", "").replace("dhx_qi_right", "") + " dhx_qi_" + (1 == e ? "left" : "right"))
    ...
    if (!this._quick_info_box) {
        ...
        t.className = "dhx_cal_quick_info", scheduler.$testmode && (t.className += " dhx_no_animate");
        var a = '<div class="dhx_cal_qi_title" style="height:' + e.quick_info_title + 'px"><div class="dhx_cal_qi_tcontent"></div><div  class="dhx_cal_qi_tdate"></div></div><div class="dhx_cal_qi_content"></div>';
        a += '<div class="dhx_cal_qi_controls" style="height:' + e.quick_info_buttons + 'px">';
        for (var r = scheduler.config.icons_select, n = 0; n < r.length; n++) a += '<div class="dhx_qi_big_icon ' + r[n] + '" title="' + scheduler.locale.labels[r[n]] + "\"><div class='dhx_menu_icon " + r[n] + "'></div><div>" + scheduler.locale.labels[r[n]] + "</div></div>";

        a += "</div>", t.innerHTML = a, dhtmlxEvent(t, "click", function(e) {
    ...
    for (var t = scheduler.getRenderedEvent(e), a = 0, r = 0, n = t; n && n != scheduler._obj;) a += n.offsetLeft, r += n.offsetTop - n.scrollTop, n = n.offsetParent;
    if (n) {
        var i = a + t.offsetWidth / 2 > scheduler._x / 2 ? 1 : 0,
            d = r + t.offsetHeight / 2 > scheduler._y / 2 ? 1 : 0;
        return {
            left: a,
            top: r,
            dx: i,
    ...
    var r = a.firstChild.firstChild;
    r.innerHTML = scheduler.templates.quick_info_title(t.start_date, t.end_date, t);
    var n = r.nextSibling;
    n.innerHTML = scheduler.templates.quick_info_date(t.start_date, t.end_date, t);
    var i = a.firstChild.nextSibling;
    i.innerHTML = scheduler.templates.quick_info_content(t.start_date, t.end_date, t)
};
